...
<div class="page-screen active-screen opened direction-screen">
    <img src="{{$direction->bannerImage()}}" class="page-screen-bg-image" />
    <div class="wraper">
        <div class="fixed-block right-block">
            <p class="fixed-block-title">{{$direction->country($activeLang)}}<br>({{$direction->translate($activeLang)->title}})</p>
        </div>
        ...
        <span class="direction-label">{{$direction->translate($activeLang)->page_label}}</span>
    </div>
</div>
<!-- end top screen -->
        ...
        <div class="row">
            <div class="col-6 offset-6">
                <div class="content-wraper">
                    <p class="page-screen-heading mb-5">{{$direction->translate($activeLang)->page_description}}</p>

                    @if($direction->options->count())
                    <div class="pluses-list row mt-2">
                        @foreach($direction->options as $directionOption)
                        <div class="col-6">
                            <div class="plus-item">
                                <div class="plus-item-heading">{{$directionOption->translate($activeLang)->title}}</div>
                                <div class="plus-item-description">{{$directionOption->translate($activeLang)->description}}</div>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            @if($direction->features->count())
                @php
                    $index = 1
                @endphp
                @foreach($direction->features as $feature)
                    @if($index <= 2)
